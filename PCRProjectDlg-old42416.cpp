...
//==================	
						//取相应的有效数据
						if ((PCRTypeFilterClass==0x01)|(PCRTypeFilterClass==0x02)|(PCRTypeFilterClass==0x03) )		//返回12 pixel 数据时
						{	
							// 区分序号
...
//							m_OperDlg.SendMessage(UM_OPERLEDPROCESS);
						...
						else if (rType == 0x27)
						{
							...
							sGraFirmwareVer.Format("FW Version: %2x.%.2x, FuncCode: %2x, ( Release date: %.2d-%.2d-20%.2d )",iGraFirmwareVer1,iGraFirmwareVer2,iFuncCode,iMonth,iDay, iYear);
						}
						break;
					}
